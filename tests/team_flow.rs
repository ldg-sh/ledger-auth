...
async fn test_team_creation_flow_success() {
    ...
    println!("[>] Verifying team creation in database for ID: {}", team_id);
...
async fn test_team_creation_flow_unauthorized() {
    ...
    let team_data = test_data::sample_team(owner_id);
    println!("[>] Sending request to create team with invalid token: {:?}", team_data);
...
async fn test_team_creation_flow_nonexistent_owner() {
    ...
    println!("[>] Sending request to create team with non-existent owner: {:?}", team_data);
...
async fn test_team_invite_flow_success() {
    ...
    println!("[>] Creating target user to invite with email: {}", target_user_email);
    ...
    println!("[<] Received response for target user creation with status: {}", resp_create.status());
    ...
    let resp = test::call_service(&app, req).await;
    println!("[<] Received response for invite with status: {}", resp.status());
...
async fn test_team_invite_flow_invite_team_owner() {
    println!("\n\n[+] Running test: test_team_invite_flow_invite_team_owner");
    let ctx = TestContext::new().await;
    let client = TestClient::new(ctx.db.clone());
    println!("[+] Test client and context created.");
    let app = test::init_service(client.create_app()).await;
    ...
    let (_admin_id, _admin_token) = client.create_test_admin().await;
    println!("[<] Admin user created.");
    println!("[>] Creating first team owner.");
    ...
    println!("[<] First team owner created with ID: {}", owner1_id);
    println!("[>] Creating team for first owner.");
    let _team1_id = client.create_team_with_owner(owner1_id).await;
    println!("[<] Team for first owner created.");

    // Create another team owner
    println!("[>] Creating second team owner.");
    ...
    println!("[<] Second team owner created with ID: {}", owner2_id);
    println!("[>] Creating team for second owner.");
    let _team2_id = client.create_team_with_owner(owner2_id).await;
    println!("[<] Team for second owner created.");

    let owner2_email = ctx.db.get_user_by_id(&owner2_id).await.unwrap().email;
    println!("[+] Got email for second owner: {}", owner2_email);

    // Try to invite another team owner
    let invite_data = RTeamInviteUser {
        user_email: owner2_email,
    };
    println!("[>] Sending invite to another team owner (should be forbidden).");
...
async fn test_accept_invite_flow_success() {
    ...
    let resp = test::call_service(&app, req).await;
    println!("[<] Received response for accept invite with status: {}", resp.status());
    ...
    println!("[>] Verifying invite was accepted in database for ID: {}", invite_id);
    ...
    println!("[>] Verifying user was moved to the team for user: {}", target_user);
    let updated_user = ctx.db.get_user_by_id(&target_user).await.unwrap();
    assert_eq!(updated_user.team_id, Some(team_id));
    println!("[<] User's team verified.");
...
async fn test_accept_invite_flow_wrong_user() {
    ...
    println!("[+] Fetched target user from DB with ID: {}", target_user.id);
    ...
    println!("[>] Sending request to accept invite with wrong user's token (should be unauthorized).");
    let req = test::TestRequest::post()
        .uri(&format!("/team/invite/accept/{}", invite_id))
...
async fn test_accept_invite_flow_invalid_invite() {
    ...
    println!("[>] Sending request to accept non-existent invite: {}", fake_invite_id);
