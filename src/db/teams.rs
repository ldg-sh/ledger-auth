...
impl PostgresService {
    ...
    pub async fn get_team(&self, id: Uuid) -> Result<TeamModel, AppError> {
        Ok(Team::find_by_id(id).one(&self.db).await?.ok_or(DbErr::RecordNotFound("Team not found".to_string()))?)
    }

    pub async fn list_teams_for_owner(&self, owner: Uuid) -> Result<Vec<TeamModel>, AppError> {
        Ok(Team::find().filter(entity::team::Column::Owner.eq(owner)).all(&self.db).await?)
    }

    pub async fn list_users_in_team(&self, team_id: Uuid) -> Result<Vec<UserModel>, AppError> {
        Ok(User::find().filter(entity::user::Column::TeamId.eq(team_id)).all(&self.db).await?)
    }
